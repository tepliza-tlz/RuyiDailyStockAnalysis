...
def create_app(static_dir: Optional[Path] = None) -> FastAPI:
    ...
    # 创建 FastAPI 实例
    app = FastAPI(
        title="RuyiDailyStockAnalysis API",
        description=(
            "如意金股 - A股/港股/美股自选股智能分析系统 API\n\n"
            "## 功能模块\n"
            "- 股票分析：触发 AI 智能分析\n"
            "- 历史记录：查询历史分析报告\n"
            "- 股票数据：获取行情数据\n\n"
...
<title>如意金股 RuyiDailyStockAnalysis - Frontend Not Built</title>
<style>
...
<p>API is running, but the 如意金股 Web UI has not been built yet.</p>
...
<p class="status">RuyiDailyStockAnalysis v1.0.0 &bull; <a href="/api/health">/api/health</a></p>
</div></body></html>"""
